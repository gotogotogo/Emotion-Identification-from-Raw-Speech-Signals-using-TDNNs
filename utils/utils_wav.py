...
# ===============================================
def load_wav(audio_filepath, min_dur_sec=10):
    audio_data, sr = torchaudio.load(audio_filepath)
    len_file = audio_data.shape[1]
    
    if len_file <int(min_dur_sec * sr):
        dummy=np.zeros((1,int(min_dur_sec*sr)-len_file))
        extened_wav = np.concatenate((audio_data,dummy), axis = 1)
    else:
...
#     extend_wav = load_wav(filepath, min_dur_sec=10)
#     extend_wav = extend_wav.reshape(1, -1)
#     return extend_wav
